    ...
    public static int entrar(Usuario usuario) throws ClassNotFoundException, SQLException {
        ...
        if (resultado.next()) {
            ...
            if (password.equals(storedpass)) {
                ...
                String nombreStore=resultado.getString("nombre");
                String apellidoStore=resultado.getString("apellido");
                int nivelaccesoStore=resultado.getInt("nivelacceso");
                int resetStore=resultado.getInt("resetpassword");
                if (stmt.executeUpdate(updateparams) == 1) {
                    usuario.setNombre(nombreStore);
                    usuario.setApellido(apellidoStore);
                    usuario.setNivelacceso(nivelaccesoStore);
                    usuario.setResetpassword(resetStore);
